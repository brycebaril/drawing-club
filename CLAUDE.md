...
Phases 1-4 are done: scaffold/schema/CI, auth + RBAC, the core booking loop, and admin user management. Built so far: registration/login/MFA, one-off session creation (`/admin/sessions`), the schedule grid with book/cancel/waitlist (`/app/schedule`), a minimal pass wallet with a dev-only test-pass grant standing in for Stripe (`/app/wallet`), and `/admin/users` (account status/ban/suspend with auto-cancellation, manual pass grants, membership adjustment, volunteer role assignment) plus `/admin/audit-logs`. Not yet built: recurring/multi-week/non-ticketed session creation, real Stripe payments, transferable pass gift/claim, CMS, public marketing pages, ops workspaces (check-in, model booking, financials), admin reporting beyond the audit log.

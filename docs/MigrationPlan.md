...
All five original open questions are resolved (see `LegacyDataAnalysis.md`'s Decisions section), plus three more surfaced while drafting the field-level mapping below (member names, username, migration traceability). One item remains genuinely pending but **does not block this plan**: a query sent to the org's former Robostrar admin about two undocumented enums (`sessions.status`, `registration_logs.what` codes 6–18) — see `LegacyDataAnalysis.md`'s Appendix. This plan uses the best-guess mappings from that analysis in the meantime and will be updated if the admin's answer changes them.
...
4. **New `legacy_attendance_history` table**, for the bulk of `seat_registrations` (25,725 rows) that represent already-completed sessions. Deliberately **separate from the live `passes`/`transactions` tables**, not a reuse of them — see §5's `seat_registrations` row for why. Proposed shape:
   - `id uuid primary key`
...
| `board_status` | **no destination in the current schema** — this app has no "Board Member" concept. Flag for the org: either drop (a Board Member becomes a plain member unless they separately hold a volunteer role) or extend `volunteer_role_name` with a new value |
...
| New column | Source | Notes |
|---|---|---|
| `status` | `sessions.status` | provisional mapping pending the admin query (§2): `10` → `'Scheduled'` (majority code, spans full history); `0`, `1`, `3`, `11` need the admin's input — **conservatively default all four to `'Canceled'` in the interim** (the safer direction to guess wrong in, since a wrongly-`Canceled` historical row costs nothing, while a wrongly-`Scheduled` one could surface a stale session in current UI if the future/historical split above has a bug) |
| `session_type` | `sessions.typeId` → `session_types` | see §6 below |
| `description` | `session_alt_descriptions`, if present for that session, else blank | legacy content is raw HTML (`<STRONG>`, `<I>`, `<BR>`) — needs conversion to plain text/markdown, not a direct copy (this app's rendering conventions avoid raw HTML — see CLAUDE.md's CMS notes) |
...
1. **Schema mapping** — *substantially complete*, see §5 above; remaining gaps are the four explicitly-flagged manual-review items (owned_passes `passKind=0` miscellaneous rows, `board_status` entitlement's missing destination, whether role passes also grant seat allowances, and the two undocumented `sessions.status`/`registration_logs.what` enum groups pending the admin's input).

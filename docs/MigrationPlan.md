...
### `legacy_registration_logs` ← `registration_logs` (75,819 of 189,527 rows)

Added 2026-08-19, after cutover, driven by a real customer-service need surfaced once the migrated data was live and the org started navigating the new admin UI: a way to look up a member's pre-cutover activity (orders, seat registrations, pass events, refunds) without needing raw SQL access. Login/logout events (`what` codes 0/1 — 113,708 of the 189,527 total rows, ~60%) are excluded at migration time; not useful for a support conversation. The full `what` vocabulary (codes 2–18) is in `LegacyDataAnalysis.md`'s Appendix.

Unlike `legacy_attendance_history`, this table uses **real foreign keys**, not denormalized text — `who`/`whom` resolve to migrated `users.id`, `whichSession` resolves to migrated `sessions.id`, and `whichOrder` resolves to migrated `transactions.id` (via a new `legacyInvoiceIdToTransactionId` map in `transactions.ts`, keyed by `store_orders.invoiceId`). All three resolve through the same in-memory legacy-id maps the rest of the migration already builds during one orchestrator run (`legacyAttendeeIdToNewId`, `legacySessionIdToNew`) — `sessions`/`store_orders` never got a persisted `legacy_id` column of their own, so **this table can only be correctly populated within that same orchestrator run, not backfilled later against an already-migrated database** (confirmed: `sessions.start_time` happens to be unique in the live data, so a natural-key backfill was technically possible, but wiring it into the orchestrator properly and re-running `--reset` was cleaner and lower-risk given no real post-cutover activity existed yet to lose). `whichPass` (rare — 1,412 rows, `owned_passes.id`, which has no live-table equivalent under the direct-ownership pass model) is kept as a bare `legacy_pass_id` integer for traceability, not a live FK.

Surfaced on `/admin/users/[id]` as a "Legacy activity" section (most recent 100 events per member, only rendered when a member actually has rows — native post-launch signups never will) — this is what makes the data usable for an actual support conversation rather than just DB-queryable, matching what was explicitly asked for alongside the migration itself.

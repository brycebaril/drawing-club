...
| Weekly volunteer free-ticket grant | Weekly (Sunday evening Pacific), via a **GitHub Actions scheduled workflow** (`.github/workflows/grant-volunteer-passes.yml`) | Grants `VOLUNTEER_WEEKLY_PASS_ALLOWANCE` ticket(s) to every `Active` member holding the `GenericVolunteer` role who currently holds fewer than `VOLUNTEER_PASS_WALLET_CAP` unspent volunteer-granted tickets (`src/lib/ops/volunteerPasses.ts`). |

The first two are implemented as EventBridge Scheduler rules invoking a protected internal API route in the Next.js app (shared-secret header, not publicly reachable) rather than a separate Lambda codebase — keeps job logic in the same codebase and deployable through the same Amplify pipeline. **The third job (added later) follows the exact same "scheduled trigger calls a protected internal API route" shape but uses a GitHub Actions cron instead of EventBridge Scheduler**, since EventBridge isn't actually provisioned for this app yet (`docs/StagingEnvironment.md`) — the route itself (`POST /api/jobs/grant-volunteer-passes`, checked against `JOB_TRIGGER_SECRET`) doesn't know or care which scheduler calls it, so swapping GitHub Actions for a real EventBridge Scheduler rule later (once that infra exists) needs no application-code change, only a trigger-side one. This job deliberately has **no admin-UI button** — an earlier version did, and it was removed on the reasoning that a real recurring weekly benefit shouldn't depend on a person remembering to click something; `/admin/passes` instead shows a standing read-only listing of grants made in the past 7 days.
